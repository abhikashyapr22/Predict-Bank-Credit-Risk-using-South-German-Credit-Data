...
class ModelTrainer:
    ...
    def train_model(self,x,y):
        try:
            # xgb_clf =  XGBClassifier(random_state=42,booster='gbtree',learning_rate='0.3',max_depth=9,n_estimators=30)
            # xgb_clf.fit(x,y)
            rf1=RandomForestClassifier(criterion='gini',max_depth=22,max_features='log2',min_samples_leaf=1,min_samples_split=2,n_estimators=120)
            rf1.fit(x,y)
            return rf1
        except Exception as e:
            raise SGCException(e, sys)
